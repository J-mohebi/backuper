...
# Function to read paths from file safely
...
# Initialize exclude and saved paths
exclude_paths=()
saved_paths=()

# Read exclude.txt if exists
if [[ -f "/root/exclude.txt" ]]; then
    echo "Reading exclude paths from /root/exclude.txt"
    exclude_paths=($(read_paths_from_file "/root/exclude.txt"))
fi

# Read saved.txt if exists
if [[ -f "/root/saved.txt" ]]; then
    echo "Reading saved paths from /root/saved.txt"
    saved_paths=($(read_paths_from_file "/root/saved.txt"))
fi

# Build exclude options for zip command
exclude_options=""
for path in "${exclude_paths[@]}"; do
    exclude_options+=" -x \"$path\""
done

# Build include options for saved paths
saved_options=""
...
zip -r /root/ac-backup-m.zip /opt/marzban/* /var/lib/marzban/* /opt/marzban/.env -x /var/lib/marzban/mysql/\* ${exclude_options}
...
EOF
      ...
      ZIP="zip -r /root/ac-backup-m.zip ${dir}/* /var/lib/marzban/* /opt/marzban/.env ${exclude_options}"
fi

# Add saved paths if any
if [[ ${#saved_paths[@]} -gt 0 ]]; then
    ZIP+=" && zip -r /root/ac-backup-m.zip ${saved_options}"
fi
...
ZIP="zip /root/ac-backup-x.zip ${dbDir}/x-ui.db ${configDir}/config.json ${exclude_options}"

# Add saved paths if any
if [[ ${#saved_paths[@]} -gt 0 ]]; then
    ZIP+=" && zip -r /root/ac-backup-x.zip ${saved_options}"
fi
...
zip /root/ac-backup-h.zip /opt/hiddify-manager/hiddify-panel/backup/\$latest_file ${exclude_options}
EOF
)

# Add saved paths if any
if [[ ${#saved_paths[@]} -gt 0 ]]; then
    ZIP+=" && zip -r /root/ac-backup-h.zip ${saved_options}"
fi
